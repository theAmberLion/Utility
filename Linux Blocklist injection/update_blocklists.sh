...
LOGFILE="/var/log/update-blocklists.log"
...
fetch_feed() {
    ...
    if ! curl -fsSL --connect-timeout 15 --max-time 60 "$URL" -o "$OUTFILE"; then
...
update_set() {
    ...
    ipset create "$TMP" hash:net family inet hashsize 8192 maxelem 30000 2>/dev/null || true

    # Parse feed and populate TMP
    if [ "$TYPE" = "json" ]; then
        # Spamhaus NDJSON parser
        jq -r '.cidr' "$WORKFILE" 2>/dev/null \
        | grep -E '^[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?$' \
        | while read -r net; do
            ipset add "$TMP" "$net" 2>/dev/null
        done
    else
        # Plain text feeds
        grep -v '^#' "$WORKFILE" \
    ...
    local COUNT
    COUNT=$(ipset list "$TMP" 2>/dev/null | awk -F': ' '/Number of entries/ {print $2}')
    ...
    if ! ipset list "$NAME" >/dev/null 2>&1; then
        ipset create "$NAME" hash:net family inet hashsize 8192 maxelem 30000
    fi
